package ru.tikhoa.pft.addressbook.generators;
...
public class ContactDataGenerator {
    ...
    private void run() throws IOException {
        List<ContactData> contacts = getGenerateContacts(count);
    ...
    private List<ContactData> getGenerateContacts(int count) {
        List<ContactData> contacts = new ArrayList<ContactData>();
        for (int i = 0; i < count; i++){
            contacts.add(new ContactData().withFirstname(String.format("name %s", i))
                    .withLastname(String.format("lastname %s", i)).withGroup("test1")
